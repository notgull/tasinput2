...
std::tuple<QWidget *, QSpinBox *> labeled_spinbox(const char *label, int min, int max) {
    // general container
...
JoystickPanel::JoystickPanel(QWidget *panel, Inputs *inputs) {
    ...
    // signals
    connect(this->x, SIGNAL(valueChanged()), this, SLOT(x_changed()));
    connect(this->y, SIGNAL(valueChanged()), this, SLOT(y_changed()));
}
