...
pub struct Debugger {
  pub debug_fn: unsafe extern fn(*mut c_void, m64p_sys::m64p_msg_level, *const c_char),
}
...
impl Write for Debugger {
  fn write_str(&mut self, s: &str) -> fmt::Result {
    ...
    unsafe { 
      (self.debug_fn)(
         *(*state_lock.context.lock().unwrap()).as_mut().unwrap().get_mut(),
